...
Checkout.prototype = {
    ...
    validateAddress: function (type) {
        ...
        var validationResult         = false,
            ...
            validator                = ('co-' + type + '-form');

        //newAddressFormValidation = validator.validate();

        var form = jQuery('#co-' + type + '-form');
        ...
        form.on('invalid.fndtn.abide', function(e) {
            ...
            var invalidFields = jQuery(this).find('[data-invalid]');
            if (invalidFields) {
                var scrollTo = jQuery('#'+invalidFields[0].id).offset().top;
    ...
    validateAgreements: function () {
        ...
        var valid = true;
        var form = jQuery('#checkout-agreements');

        form.foundation('validateForm');

        if ($$('#checkout-agreements input[name="agreement"]').length < 0) {
            valid = false;
        }
    ...
    validateReview: function (validateSteps) {
        ...
        if (validateSteps) {
            valid = this.validateCheckoutSteps(
                ['CheckoutMethod', 'BillingAddress', 'ShippingAddress', 'ShippingMethod', 'PaymentMethod', 'Agreements']
            );
        } else {
            valid = true;
        }
