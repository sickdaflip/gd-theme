...
class Gastrodax_Sitemap_Model_Sitemap extends Mage_Sitemap_Model_Sitemap
{
    ...
    {
        $enabled = (bool) Mage::getStoreConfig('gastrodax_sitemap/general/enabled');
        if(!$enabled) {
            return parent::generateXml();
        }
        $helper = Mage::helper('gastrodax_sitemap');

        $limit = (int) Mage::getStoreConfig('gastrodax_sitemap/general/limit');
        if ($limit == 0) {
